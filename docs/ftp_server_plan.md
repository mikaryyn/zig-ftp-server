...
- [x] (2026-02-07 23:16Z) Milestone 10: `STOR` streaming upload with tests and manual smoke test.
- [x] (2026-02-07 23:42Z) Milestone 11: File ops (`DELE`, `RNFR`/`RNTO`) + optional ops (`MKD`, `RMD`, `SIZE`, `MDTM`) implemented with sequencing/error-map coverage.
- [x] (2026-02-08 00:22Z) Milestone 12: Timeouts + limits hardening + error→reply mapping audit + integration tests.
- [x] (2026-02-08 00:45Z) Milestone 13: Documentation polish (README + docs) + final acceptance run-through.
...
- Milestone 10 delivered non-blocking streaming `STOR` in `src/ftp/server.zig` and `src/ftp/transfer.zig`: `STOR <path>` now validates path args, opens `Fs.openWriteTrunc`, waits for PASV accept before replying `150`, streams bytes from data `Net.read` into `Fs.writeFile` with partial file-write resume, treats `read=0`/`error.Closed` as upload EOF, then closes writer/data/listener and replies `226`. `src/ftp/mock_vfs.zig` now captures uploaded bytes for deterministic tests, and `src/cli/vfs_os.zig` now implements OS-backed `openWriteTrunc`/`writeFile`/`closeWrite` so CLI uploads work against real files. Automated validation passed via `~/zig/zig build test`.
- Milestone 11 delivered stateful file operations in `src/ftp/server.zig`: `DELE` now calls `Fs.delete`, `RNFR`/`RNTO` now enforce strict sequencing (`503` while pending except `RNTO`/`QUIT`) and perform `Fs.rename`, and optional commands `MKD`/`RMD`/`SIZE`/`MDTM` are now implemented when supported by `Fs` (otherwise `502`). `src/cli/vfs_os.zig` now implements OS-backed delete/rename/mkdir/rmdir/size/mtime, `src/ftp/mock_vfs.zig` now provides deterministic behavior for these commands, and `FEAT` now advertises `SIZE`/`MDTM` only when present. Automated validation passed via `~/zig/zig build test`.
- Milestone 12 delivered deterministic timeout hardening in `src/ftp/server.zig`: optional control idle timeout now closes inactive control sessions, PASV idle timeout now closes unused passive listeners, and transfer idle timeout now aborts stalled transfers with `426` after no forward progress. Core path validation now rejects NUL bytes and overlong path arguments (`limits.path_max`) before dispatching to VFS across path-based commands, while preserving existing `553` reply mapping. Added targeted unit coverage for control/PASV/transfer timeout behavior and path-limit rejection; automated validation passed via `~/zig/zig build test`.
- Milestone 13 delivered documentation polish and acceptance evidence: `README.md` now reflects the implemented CLI/runtime (using `~/zig/zig`), command support, and MVP limitations; `docs/net_fs_implementation.md` now documents how to implement `Net`/`Fs` and clarifies `tick()` non-blocking/cooperative guarantees with partial-I/O expectations. Final acceptance run-through covered auth success/failure, `PWD`/`CWD`/`LIST`/`STOR`/`RETR`/`DELE`/`RNFR`/`RNTO`/`TYPE`/`SYST`/`FEAT`, unsupported `PORT` (`502`), and concurrent-session rejection (`421`), with automated validation via `~/zig/zig build test` and `~/zig/zig build`.
...
Plan Update Notes (2026-02-07): Completed Milestone 10 by implementing non-blocking `STOR` streaming with accept-gated `150`, partial VFS write resume, and `226` completion; added mock VFS upload capture plus server tests for STOR sequencing/partial writes/fs-error mapping, and OS VFS `openWriteTrunc`/`writeFile`/`closeWrite` support; validated with `~/zig/zig build test`.
Plan Update Notes (2026-02-07): Completed Milestone 11 by implementing `DELE`, strict `RNFR`/`RNTO`, and optional `MKD`/`RMD`/`SIZE`/`MDTM` command handlers, extending mock/OS VFS backends for those operations, updating FEAT advertisement for optional capabilities, and adding sequencing/error-mapping tests validated with `~/zig/zig build test`.
Plan Update Notes (2026-02-08): Completed Milestone 12 by implementing deterministic control/PASV/transfer idle timeout handling, adding core path-argument NUL/length guards (`path_max`) across path-based commands, and extending server tests with timeout and path-limit coverage validated via `~/zig/zig build test`.
Plan Update Notes (2026-02-08): Completed Milestone 13 by updating `README.md` and adding `docs/net_fs_implementation.md`, then running final acceptance checks across auth, navigation, transfers, file ops, unsupported `PORT`, and second-connection `421`, alongside `~/zig/zig build test` and `~/zig/zig build`.
